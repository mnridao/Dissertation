...
def plotWithAnalytical1(grid, t, u):
    ...
    phiAnaly = analy.analyticalSolution1(helpers.middleWaveIC, 
                                         sPhiCoeff, 
                                         u, 
                                         solver.grid.X, 
                                         t)
    ...
    # Plot the real part on the left subplot
    axs[0].plot(solver.grid.X, phiAnaly.real, 'k--')
    axs[0].plot(solver.grid.X, solver.grid.phi.real)
    axs[0].set_xlabel('X', fontsize=fontsize)
    axs[0].set_ylabel('b', fontsize=fontsize)
    axs[0].grid(True)
    ...
    # Plot the imaginary part on the right subplot   
    axs[1].plot(solver.grid.X, phiAnaly.imag, 'k--')     
    axs[1].plot(solver.grid.X, solver.grid.phi.imag)
    axs[1].set_xlabel('X', fontsize=fontsize)
    axs[1].set_ylabel('Nw', fontsize=fontsize)
    axs[1].grid(True)
...
def plotWithAnalytical3(grid, t, u=None):
    ...
    # Calculate the analytic solution.
    ic = lambda x: (1+1j)*np.zeros_like(x, dtype=np.complex128)
    phiAnaly = analy.analyticalSolution3(ic, sPhiCoeff, grid.X, t)
    ...
    axs[0].set_ylabel('b', fontsize=fontsize)
    axs[0].grid(True)
    axs[0].set_ylim([-1e-4, 1e-4]) # Doesn't matter for this one.
...
def plotWithAnalytical4(solver):
    ...
    phiAnaly = solver.getCustomEquation("analytic").I1
    ...
    # Plot the real part on the left subplot
    axs[0].plot(solver.grid.X/1e3, phiAnaly.real, 'k--')
    axs[0].plot(solver.grid.X/1e3, solver.grid.phi.real)
    axs[0].set_xlabel('X [km]', fontsize=fontsize)
    axs[0].set_ylabel('b', fontsize=fontsize)
    axs[0].grid(True)
    axs[0].set_ylim([-1e-6, 1e-6])
    ...
    # Plot the imaginary part on the right subplot   
    axs[1].plot(solver.grid.X/1e3, phiAnaly.imag, 'k--')     
    axs[1].plot(solver.grid.X/1e3, solver.grid.phi.imag)
    axs[1].set_xlabel('X [km]', fontsize=fontsize)
    axs[1].set_ylabel('Nw', fontsize=fontsize)
    axs[1].grid(True)
    axs[1].set_ylim([-1e-6, 1e-6])
